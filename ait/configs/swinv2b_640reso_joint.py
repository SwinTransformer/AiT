    ...
    insseg=dict(  # len=117266
        ...
        data=dict(
            train=dict(type=det_dataset_type, ann_file='data/coco/annotations/instances_train2017.json',
                       img_prefix='data/coco/train2017/', pipeline=insseg_train_pipeline, samples_total_gpu=60),
            val=dict(type=det_dataset_type, ann_file='data/coco/annotations/instances_val2017.json', img_prefix='data/coco/val2017/', pipeline=insseg_test_pipeline, samples_per_gpu=8, workers_per_gpu=4),)
    ),
    depth=dict(  # len=24231
        times=1,
        data=dict(
            train=dict(type='nyudepthv2', data_path='data', filenames_path='code/dataset/depth/filenames/',
                       is_train=True, crop_size=(480, 480), samples_total_gpu=4),
