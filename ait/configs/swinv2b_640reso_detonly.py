checkpoint_config = dict(interval=1833 * 5)  # 5 epoch
log_config = dict(
    interval=20,
    hooks=[
...
runner = dict(type='IterBasedRunnerMultitask', max_iters=183300)  # 100 epoch
evaluation = dict(interval=18330)  # 10 epoch
...
det_train_pipeline = [
    ...
    dict(type='AddKey', kv={'task_type': 'det'})
]
    ...
    det=dict(
        ...
        data=dict(
            train=dict(type=det_dataset_type, ann_file='data/coco/annotations/instances_train2017.json',
                       img_prefix='data/coco/train2017/', pipeline=det_train_pipeline, samples_total_gpu=128),
            val=dict(type=det_dataset_type, ann_file='data/coco/annotations/instances_val2017.json',
                     img_prefix='data/coco/val2017/', pipeline=det_test_pipeline, samples_per_gpu=20, workers_per_gpu=2),
            test=dict(type=det_dataset_type, ann_file='data/coco/annotations/instances_val2017.json', img_prefix='data/coco/val2017/', pipeline=det_test_pipeline, samples_per_gpu=20, workers_per_gpu=2))
    )
)
